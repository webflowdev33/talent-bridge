...
export default function Jobs() {
  ...
  const filteredJobs = jobs.filter(job => {
    ...
    const matchesCampaign = selectedCampaign === 'all' || job.campaign_id === selectedCampaign;
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-2xl mx-auto">
              ...
              {campaigns.length > 0 && (
                <Select value={selectedCampaign} onValueChange={setSelectedCampaign}>
                  <SelectTrigger className="h-14 bg-white border-0 shadow-xl text-base w-full sm:w-[200px]">
              ...
              <div className="flex items-center justify-between mb-10">
                <p className="text-foreground text-lg">
                  ...
                  {selectedCampaign !== 'all' && campaigns.find(c => c.id === selectedCampaign) && (
                    <span className="text-muted-foreground"> in {campaigns.find(c => c.id === selectedCampaign)?.name}</span>
                  )}
                </p>
              </div>
              <div className="grid gap-6">
                {filteredJobs.map((job) => {
                  const campaignName = getCampaignName(job.campaign_id);
